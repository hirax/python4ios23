...
# フォトライブラリーから静止画URLを取得する場合
@on_main_thread
def pick_url_from_photolibrary(url_dic):
    ...
    def imagePickerController_didFinishPickingMediaWithInfo_(self, cmd, picker, info):
        ...
        ObjCInstance(self).release()
        pick.dismissViewControllerAnimated_completion_(True, None) # Dismiss the sheet:
        infos = ObjCInstance(info)  # Get UIImage info
        #url_dic['url'] = infos['UIImagePickerControllerMediaURL']
        url_dic['url'] = infos['UIImagePickerControllerReferenceURL'] # iOS11以降deprecated
        print(url_dic['url'])
    # UIImagePickerControllerDelegate を登録する
    MyPickerDelegate = create_objc_class(
        'MyPickerDelegate',
...
@on_main_thread
def pick_url_from_photoalbum(url_dic):
    # 画像選択後に呼ばれるdelegateを定義
    def imagePickerController_didFinishPickingMediaWithInfo_(self, cmd, picker, info):
        pick = ObjCInstance(picker)
        pick.setDelegate_(None)  # Set delegate to nil, and release its memory:
