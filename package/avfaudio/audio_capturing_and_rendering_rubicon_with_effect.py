...
# https://developer.apple.com/documentation/avfaudio/audio_engine/audio_units?language=objc
# Audio effects
AVAudioUnitEffect = rubicon_api.ObjCClass('AVAudioUnitEffect')
AVAudioUnitEQ = rubicon_api.ObjCClass('AVAudioUnitEQ')
AVAudioUnitDistortion = rubicon_api.ObjCClass('AVAudioUnitDistortion')
AVAudioUnitDelay = rubicon_api.ObjCClass('AVAudioUnitDelay')
AVAudioUnitReverb = rubicon_api.ObjCClass('AVAudioUnitReverb')

# Time effects
AVAudioUnitTimeEffect = rubicon_api.ObjCClass('AVAudioUnitTimeEffect')
AVAudioUnitTimePitch = rubicon_api.ObjCClass('AVAudioUnitTimePitch')
AVAudioUnitVarispeed = rubicon_api.ObjCClass('AVAudioUnitVarispeed')

# Audio generation
AVAudioUnitEQ = rubicon_api.ObjCClass('AVSpeechSynthesisProviderAudioUnit')
...
def rendering_function(t, channel): # channel→0(右)、1(左)
    if channel == 0:
        ...
        return 1.0 * math.sin(880.0 * 2.0 * math.pi * t)

class AVAudio_Capture_and_Rendering_and_Effects:
    ...
    def __init__( self,
                  ...
                  avaudio_units              # マイク入力にエフェクトを掛けた上で音声出力する場合
                ):
        ...
        if (user_rendering_function is not None) and (channels_for_rendering is not None):
            ...
            def render_block_function_with_channel(
                ...
                for frame in range(frame_count):
                    # 外部関数で値設定
                    val = user_rendering_function(self.time, channel=channel)
                    if channel == 0:  # channnel 0 の場合のみ時間を進ませる（二重処理を防ぐ）
            ...
            for channel in range(self.channels):
                ...
                if channel == 0: # 右
                    func = format_for_output, render_block_function_0
                if channel == 1: # 左
                    func = format_for_output, render_block_function_1
                self.source_nodes[channel].initWithFormat_renderBlock_(format_for_output, func)
                self.AVAudioEngine.attachNode(self.source_nodes[channel]) # AVAudioEngineに追加
        ...
        # ---------- マイク入力にエフェクトを掛けて出力する場合（ここから） ------------
        #distortion = AVAudioUnitDistortion()
        #reverb = AVAudioUnitReverb()
        # avaudio_units = []
        if avaudio_units is not None: # エフェクト処理が与えられていたら
            self.avaudio_units = avaudio_units
            input_node = self.AVAudioEngine.inputNode     # 暗黙入力ノード
            mixer_node = self.AVAudioEngine.mainMixerNode # 暗黙のMixerノード
            format_for_output_node = mixer_node.inputFormatForBus(0)

            # マイク入力をステレオ中央定位にするためにサブミキサーを作る
            self.sub_mixer_node = AVAudioMixerNode.alloc().init()
            self.AVAudioEngine.attachNode(self.sub_mixer_node)
            # マイク入力をサブミキサーに繋ぐ
            # フォーマットをNoneにすることで、マイク入力が片側モノラルになることを防ぐ
            self.AVAudioEngine.connect_to_format_( input_node,
                        self.sub_mixer_node, None)

            #self.AVAudioEngine.connect_to_toBus_format_( input_node,
            #            self.sub_mixer_node, 0, format_for_output_node)
            #self.AVAudioEngine.connect_to_toBus_format_( input_node,
            #            self.sub_mixer_node, 1, format_for_output_node)

            #print(format_for_output_node)
            for unit in self.avaudio_units:                # 各エフェクトをAVAudioEngineに追加
                self.AVAudioEngine.attachNode(unit)
            for i, unit in enumerate(self.avaudio_units):  # ノード間を繋ぐ
                if i == 0: # サブミキサーから効果処理に繋ぐ
                    #self.AVAudioEngine.connect_to_format_( input_node,
                    #        self.avaudio_units[i], format_for_output_node)
                    self.AVAudioEngine.connect_to_format_( self.sub_mixer_node,
                        self.avaudio_units[i], format_for_output_node)
                else:      # 効果処理間を繋ぐ
                    self.AVAudioEngine.connect_to_format_( self.avaudio_units[i-1],
                            self.avaudio_units[i], format_for_output_node)
            # 処理結果末尾をmixerに繋ぐ
            self.AVAudioEngine.connect_to_format_( self.avaudio_units[-1],
                                        mixer_node, format_for_output_node)
        # ---------- マイク入力にエフェクトを掛けて出力する場合（ここまで） ------------
    ...
    def start(self):
        ...
        #self.AVAudioEngine.inputNode.pan = 0.0
        #self.sub_mixer_node.pan = 0.0
        #self.avaudio_units[-1].pan = 0.0
